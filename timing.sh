...
echo "workers,grid,#snowmen,seconds,performance(1/s),speedup,ideal_speedup" > "$SS_CSV"
echo "workers,grid,#snowmen,seconds,performance(1/s),efficiency,speedup" > "$WS_CSV"

#secs() { #Times runtime and then converts "real" to seconds
#    ( time mpirun -n "$1" ./snowman "$2" "$3" >/dev/null ) 2>&1 \
#    | awk '/^real/ { sub(/m/, " "); sub(/s/, ""); print $2*60 + $3 }'
#}
avg_time() {
  LC_ALL=C mpirun -n "$1" ./snowman "$2" "$3" \
    | awk '/^Avg Local Computation Time/ {print $(NF-1)}'
}
...
perf-report -o="$REPD" --mpi mpirun -n  1 ./snowman "$SS_N" "$SS_MEN"
perf-report -o="$REPD" --mpi mpirun -n 32 ./snowman "$SS_N" "$SS_MEN"
perf-report -o="$REPD" --mpi mpirun -n 96 ./snowman "$SS_N" "$SS_MEN" 
echo "CSV: $SS_CSV"
echo "CSV: $WS_CSV"
echo "Reports: $REPD"
